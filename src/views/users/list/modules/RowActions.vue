...
const { isPending: isDeletePending, mutate: performDelete } = useMutation({
  ...
  onError: (error) => {
    console.error('performDelete mutation failed:', error)
    toast.error(t('error_occurred'))
  }
})
      ...
      <DialogContent class="sm:max-w-[460px] !rounded-xl p-5 gap-0 border-none">
        ...
        <div class="flex items-center justify-end gap-3 pt-10">
          <Button
            ...
            class="h-10 px-5 border border-red-200 text-[#ef4444] hover:bg-red-50 hover:text-[#ef4444] rounded-lg font-semibold text-sm shadow-none cursor-pointer transition-all"
          >
            {{ t('delete') }}
          </Button>
          <Button
