...
const {
  regionId: userRegionId, cityId: userCityId, schoolId: userSchoolId,
...
watch(filterRegionId, () => {
  ...
  filterSchoolId.value = undefined
...
const fetchLiveEvents = async () => {
  ...
  try {
    ...
    liveEvents.value = data
      .sort((a: any, b: any) =>
        new Date(b.attendanceTime || b.comingTime || b.date || 0).getTime() -
        new Date(a.attendanceTime || a.comingTime || a.date || 0).getTime()
      )
      .slice(0, 5)
  } catch {
    // silent
...
const schoolBarOptions = computed(() => ({
  ...
  yaxis: {
    max: 100,
    labels: { formatter: (v: number) => v + '%', style: { fontSize: '11px', colors: '#9ca3af' } }
  },
      ...
      donut: {
        ...
        labels: {
          ...
          total: {
            show: true,
            label: t('all', 'Jami'),
            formatter: () => String(camTotal.value)
          }
        }
      }
    }
...
const cameraDonutSeries = computed(() => [
  camOnline.value || 0, camOffline.value || 0, camError.value || 0
])
...
const formatTime = (raw: any) => {
  ...
  return d.toLocaleTimeString(locale.value === 'uz' ? 'uz-UZ' : 'ru-RU', {
...
const liveStatusColor = (type: string) => {
  ...
  return (low.includes('exit') || low.includes('out') || low.includes('ket'))
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          <div class="flex items-center justify-between mb-4">
            ...
            <button v-if="selectedSchoolId" @click="selectedSchoolId = undefined; selectedSchoolName = ''"
              ...
              <XIcon class="w-3 h-3" />{{ t('reset', 'Bekor') }}
            </button>
          </div>
