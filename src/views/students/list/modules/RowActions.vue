...
import { deleteStudent, changeStudentStatus } from '../api'
...
const { isPending: isDeletePending, mutate: performDelete } = useMutation({
  mutationFn: () => {
    return deleteStudent(props.student.id)
  },
  onSuccess: () => {
    toast.success(t('student_deleted', "O'quvchi muvaffaqiyatli o'chirildi"))
    isDeleteDialogOpen.value = false
    queryClient.invalidateQueries({ queryKey: ['students'] })
  },
  onError: (error: any) => {
    console.error('deleteStudent mutation failed:', error)
...
// Status toggle: Active <-> Inactive (Deleted is terminal, handled by delete).
const isActive = computed(() => (props.student.status ?? EntityStatus.Active) === EntityStatus.Active)
const isDeletedStatus = computed(() => (props.student.status ?? EntityStatus.Active) === EntityStatus.Deleted)

const { isPending: isStatusPending, mutate: performStatusChange } = useMutation({
  mutationFn: (next: number) => changeStudentStatus(props.student.id, next),
  onSuccess: () => {
    toast.success(t('status-updated', 'Holat yangilandi'))
    queryClient.invalidateQueries({ queryKey: ['students'] })
  },
  onError: (error: any) => {
    console.error('changeStudentStatus failed:', error)
    toast.error(t('error_occurred'))
  }
})
...
<template>
  <div class="flex items-center gap-2" @click.stop>
    <!-- Edit Button (Green) -->
    <button @click="handleEdit" :title="t('edit-student', 'O\'quvchini tahrirlash')"
    ...
    <!-- Delete Button (Red) -->
    <button @click="isDeleteDialogOpen = true" :title="t('delete-student', 'O\'quvchini o\'chirish')"
      ...
      <DialogContent class="sm:max-w-[460px] !rounded-xl p-5 gap-0 border-none bg-white">
        <DialogHeader class="border-b border-gray-300 pb-3 text-left">
          <DialogTitle class="text-base sm:text-lg font-semibold text-[#1b1b1b] -mt-1">
            {{ t('delete-student', "O'quvchini o'chirish") }}
          </DialogTitle>
        </DialogHeader>
        <!-- Custom Close Button -->
        ...
        <div class="py-7 text-center">
          <p class="text-base sm:text-base text-[#1b1b1b] leading-relaxed font-semibold max-w-[280px] sm:max-w-[380px] mx-auto">
            {{ t('confirm-delete-student-msg', "Siz haqiqatdan ham ushbu o'quvchini o'chirib yubormoqchimisiz?") }}
          </p>
        </div>
