      ...
      <TableHeader class="sticky top-0 bg-white drop-shadow-sm">
        <TableRow v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
          <TableHead
            ...
            :key="header.id"
            :class="[
              'text-nowrap text-sm cursor-pointer select-none border border-t-0 p-4 pl-4 first:text-center first:pl-2 relative',
              index === 0 ? 'border-l-0' : '',
          ...
          >
            ...
            <template v-if="['paymentNumber', 'date', 'amount'].includes(header.column.id)">
              <span
      ...
      <TableBody>
        ...
        <template v-else-if="table.getRowModel().rows.length">
          <TableRow
            ...
            @click.stop="emit('row-click', row.original)"
          >
            <TableCell
              v-for="(cell, cellIndex) in row.getVisibleCells()"
              :key="cell.id"
              :class="[
                'border p-4 font-medium pl-4 first:text-center first:pl-2',
