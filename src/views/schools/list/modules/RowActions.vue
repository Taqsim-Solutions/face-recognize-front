...
import { deleteSchool, changeSchoolStatus } from '../api'
...
import EditSchoolDrawer from './EditSchoolDrawer.vue'
...
const { isPending: isDeletePending, mutate: performDelete } = useMutation({
  mutationFn: () => {
    return deleteSchool(props.school.id)
  },
  onSuccess: () => {
    toast.success(t('school_deleted'))
    isDeleteDialogOpen.value = false
    queryClient.invalidateQueries({ queryKey: ['schools'] })
  },
  onError: (error) => {
    console.error('deleteSchool mutation failed:', error)
...
const isActive = computed(() => ((props.school as any).status ?? EntityStatus.Active) === EntityStatus.Active)
const isDeletedStatus = computed(() => ((props.school as any).status ?? EntityStatus.Active) === EntityStatus.Deleted)

const { isPending: isStatusPending, mutate: performStatusChange } = useMutation({
  mutationFn: (next: number) => changeSchoolStatus(props.school.id, next),
  onSuccess: () => {
    toast.success(t('status-updated', 'Holat yangilandi'))
    queryClient.invalidateQueries({ queryKey: ['schools'] })
  },
  onError: (error: any) => {
    console.error('changeSchoolStatus failed:', error)
    toast.error(t('error_occurred'))
  }
})
...
<template>
  <div class="flex items-center gap-2" @click.stop>
    ...
    <!-- Delete Button (Red) -->
    <button @click="isDeleteDialogOpen = true" title="Delete"
    ...
    <!-- Edit School Drawer -->
    <EditSchoolDrawer v-model:open="isEditOpen" :school="props.school" />
  </div>
</template>
