...
const emit = defineEmits(['update:pagination', 'rowClick'])
...
<template>
  <div class="relative border rounded-t-lg w-full overflow-auto">
      ...
      <TableHeader class="sticky top-0 bg-white drop-shadow-sm">
        <TableRow v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
          <TableHead
            ...
            :key="header.id"
            :class="[
              'text-nowrap text-sm cursor-pointer select-none border border-t-0 p-3 pl-4 h-16 first:pl-5 relative',
              index === 0 ? 'border-l-0' : '',
  ...
  <div
    class="flex items-center !mt-0 justify-between border border-t-0 rounded-b-lg px-4 py-3 text-sm text-gray-600"
  >
    <!-- Left side -->
