...
import { updateGovernment, fetchRegions } from '../api'
...
// Zod Schema matching update fields for governments
...
// Prefill form when drawer opens or employee changes
watch(
  () => props.open,
  (isOpenVal) => {
    if (isOpenVal && props.employee) {
      ...
      resetForm({
        values: {
          ...
          level: Number(props.employee.level) || 4,
          regionId: props.employee.region?.id ? Number(props.employee.region.id) : (Number(props.employee.regionId) || undefined as any),
          cityId: props.employee.level === 4 ? undefined : (props.employee.city?.id ? Number(props.employee.city.id) : (Number(props.employee.cityId) || undefined))
        }
      })
      nextTick(() => {
...
watch(() => values.level, (newLevel, oldLevel) => {
  if (isPrefilling.value) return
  // Only clear if this is an actual user change and not the initial fill
  if (oldLevel !== undefined && newLevel === 4) {
    setFieldValue('cityId', undefined)
  }
})

watch(() => values.regionId, (_, oldReg) => {
  if (isPrefilling.value) return
  if (oldReg !== undefined) {
    setFieldValue('cityId', undefined)
  }
...
const onSubmit = handleSubmit((formValues) => {
  ...
  mutate({ id: props.employee.id, payload })
})
    ...
    <SheetContent side="right" class="w-full sm:max-w-[500px] flex flex-col p-0 bg-white [&>button]:hidden">
      <SheetHeader
        ...
        <SheetTitle class="text-[17px] font-semibold text-[#1b1b1b]">
          {{ t('edit-government', 'Hokimni tahrirlash') }}
        </SheetTitle>
        <SheetClose
