...
const { isPending: isDeletePending, mutate: performDelete } = useMutation({
  ...
  onError: (error) => {
    console.error('deleteGovernment mutation failed:', error)
...
<template>
  <div class="flex items-center gap-2" @click.stop>
      ...
      <DialogContent class="sm:max-w-[460px] !rounded-xl p-5 gap-0 border-none bg-white">
        <DialogHeader class="border-b border-gray-300 pb-3 text-left">
          <DialogTitle class="text-base sm:text-lg font-semibold text-[#1b1b1b] -mt-1">
            {{ t('delete-government', 'Hokimni o\'chirish') }}
          </DialogTitle>
        </DialogHeader>
        <!-- Custom Close Button -->
        ...
        <div class="py-7 text-center">
          <p
            ...
            {{ t('confirm-delete-govt-msg', 'Siz haqiqatdan ham ushbu hokimni o\'chirib yubormoqchimisiz?') }}
          </p>
        </div>
    ...
    <!-- Edit Government Drawer -->
    <EditGovernmentDrawer v-model:open="isEditOpen" :employee="props.employee" />

    <!-- Update Password Drawer -->
    <UpdatePasswordDrawer v-model:open="isPasswordOpen" :employee="props.employee" />
  </div>
</template>
