...
const table = useVueTable({
  ...
  meta: {
    deleteLeave: (id: string) => props.onDelete?.(id)
  }
})
...
<template>
  <div class="relative border rounded-t-lg w-full overflow-auto lg:max-h-[calc(100vh-248px)]">
      ...
      <TableHeader class="sticky top-0 bg-white drop-shadow-sm">
        <TableRow v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
          <TableHead
            ...
            :key="header.id"
            :class="[
              'text-nowrap text-sm cursor-pointer select-none border border-t-0 p-3 pl-4 first:text-center first:pl-2 relative',
              index === 0 ? 'border-l-0' : '',
        ...
        <template v-else-if="table.getRowModel().rows.length">
          ...
          >
            <TableCell
              ...
              :key="cell.id"
              :class="[
                'border p-5 font-medium pl-4 first:text-center first:p-0',
  ...
  >
    ...
    <div class="flex items-center space-x-2">
      <label for="page-size-select" class="text-[#596881]">{{ t('showing') }}</label>
