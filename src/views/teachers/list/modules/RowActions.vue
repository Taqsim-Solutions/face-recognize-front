...
import { deleteTeacher, changeTeacherStatus } from '../api'
...
const isActive = computed(() => ((props.teacher as any).status ?? EntityStatus.Active) === EntityStatus.Active)
const isDeletedStatus = computed(() => ((props.teacher as any).status ?? EntityStatus.Active) === EntityStatus.Deleted)

const { isPending: isStatusPending, mutate: performStatusChange } = useMutation({
  mutationFn: (next: number) => changeTeacherStatus(props.teacher.id, next),
  onSuccess: () => {
    toast.success(t('status-updated', 'Holat yangilandi'))
    queryClient.invalidateQueries({ queryKey: ['teachers'] })
  },
  onError: (error: any) => {
    console.error('changeTeacherStatus failed:', error)
    toast.error(t('error_occurred'))
  }
})
